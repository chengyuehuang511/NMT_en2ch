...
def evaluate1(dataloader, model, device):
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(dataloader):
            ...
            cn_sent = batch.trg_text
            src = batch.src.clone()
            src_mask = batch.src.clone()
            src_mask[src_mask!=0] = 1
            decode_result = model.generate(src, attention_mask=src_mask, max_length=src.shape[1])
            translation = [sp_chn.decode_ids(_s.tolist()) for _s in decode_result]
            trg.extend(cn_sent)
            res.extend(translation)
    trg = [trg]
    bleu = sacrebleu.corpus_bleu(res, trg, tokenize='zh')
